...
public class CustomAuthorizationFilter extends OncePerRequestFilter{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if(request.getServletPath().equals("/auth/login") || request.getServletPath().equals("/auth/token/refresh")) {
			//el usuario no hace nada solo iniciara sesion
			filterChain.doFilter(request, response);
		}else {
			//encabezado de autorizacion especifico que deberia ser la clave para el token
			//Da la autorizacion
			...
			if(authorizationHeader != null && authorizationHeader.startsWith("Bearer ")) {
				try {
					...
					//Se observa el rol del usuario y determina que recurso puede acceder
